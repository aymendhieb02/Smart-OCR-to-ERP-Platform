...
def load_document(path: Path, original_filename: str | None = None) -> LoadedDocument:
    ...
    if extension == ".pdf":
        return _load_pdf(path, original_filename or path.name)
    return _load_image(path, original_filename or path.name)


def _load_pdf(path: Path, source_file: str) -> LoadedDocument:
    doc = fitz.open(path)
    text_parts: list[str] = []
...
def _load_image(path: Path, source_file: str) -> LoadedDocument:
